...
namespace RA
{
    public class ResponseContext
    {
        ...
        {
            return _headers.Where(x => x.Key.Equals(key, StringComparison.OrdinalIgnoreCase))
        ...
        {
            return _loadValues.Where(x => x.Key.Equals(key, StringComparison.OrdinalIgnoreCase))
                    .Select(x => x.Value)
                    .DefaultIfEmpty(0)
                    .FirstOrDefault(); 
        }
        ...
        public ResponseContext Debug()
        {
            ...
            if (_loadResponses.Any())
            {
                ...
                LoadValueTypes.All.ForEach(x => "{0} {1}".WriteLine(_loadValues[x.Value], x.DisplayName.ToLower()));
            }
